...
# Simulation G/M/1 (temps inter-arrivées ~ Gamma)
def simulation_gm1(taux_arrivee, taux_service, n):
     # Arrivées suivent une loi Gamma (plus générale que l'exponentielle)
    temps_arrivees = np.cumsum(np.random.gamma(shape=2, scale=1 / (2 * taux_arrivee), size=n))
...
for taux_lambda in LAMBDA_LISTE:
    ...
    for _ in range(NB_REPETS):
        moy_reponse, moy_attente = simulation_gm1(taux_lambda, TAUX_SERVICE, NB_CLIENTS)
...
plt.title("📊 Résultats de la simulation G/M/1", fontsize=16, color='white')
...
plt.tight_layout()
plt.savefig("simulation_gm1.png", dpi=300, facecolor='black')
plt.show()
