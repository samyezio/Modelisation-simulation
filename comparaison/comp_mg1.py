...
results_mg1_det = {"lambda": [], "response_time": [], "wait_time": [], "rho": []}
...
for lmbda in LAMBDA_VALUES:
    ...
    for _ in range(REPEATS):
        resp, wait = simulate(
            arrival_gen=lambda n: np.random.exponential(1/lmbda, n),
            service_gen=lambda n: np.full(n, 1 / MU),
            n=NUM_CUSTOMERS
        )
        responses.append(resp)
        waits.append(wait)
    results_mg1_det["lambda"].append(lmbda)
    results_mg1_det["response_time"].append(np.mean(responses))
    results_mg1_det["wait_time"].append(np.mean(waits))
    results_mg1_det["rho"].append(lmbda / MU)   
...
plot_results(results_mg1_det, "M/G/1_det ")
plt.title("📊 Comparaison des modèles M/G/1 et M/G/1_det", fontsize=16, color='white')
...
plt.tight_layout()    
plt.savefig("comp_mg1.png", dpi=300, facecolor='black')
plt.show()    